...
class MediaRepository {
    ...
    fun getImagesGroupedByDate(contentResolver: ContentResolver): Pair<MutableList<ImageGroup>, MutableList<PhotoItem>> {
    ...
    fun getAudioList(context: Context): MutableList<Audio>{
        ...
        contentResolver.query(queryUri, projection, null, null, sortOrder)?.use { cursor ->
            ...
            while (cursor.moveToNext()) {
                ...
                val audio = Audio(id, displayName, path, formatDuration(duration.toLong()), formatFileSize(size), isMusic == 1, uri, thumbnailBitmap, formatAddedTime(dateAdded))
                list.add(audio)
            }
        }
        return list
    ...
    @RequiresApi(Build.VERSION_CODES.Q)
    fun fetchVideoGroups(context: Context): Pair<List<VideoItem>, List<VideoGroup>> {
        ...
        query?.use { cursor ->
            ...
            while (cursor.moveToNext()) {
                ...
                videoList.add(VideoItem(id, displayName, dateAdded, contentUri, thumbnail, formatFileSize(size)))
            }
